...
class Alaug(object):
    ...
    def __call__(self, data):
        ...
        # for lane in data_aug['gt_points']:
        #     lane_y = np.array(lane[1::2])
        #     lane_x = np.array(lane[0::2])
        #     if not np.all(lane_y[1:] < lane_y[:-1]):
        #         print("lane y", lane_y)
        #         print("lane x", lane_x)
        #         img = data_aug['img']
        #         points = np.array(lane).reshape(-1, 2).astype(int)
        #         for i in range(len(points) - 1):
        #             cv2.line(img, tuple(points[i]), tuple(points[i + 1]), color=(0, 255, 0), thickness=2)
        #         output_image_path = "prob.jpg"
