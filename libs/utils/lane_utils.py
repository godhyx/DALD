...
def sample_lane(points, sample_ys, img_w, img_h, keypoint):
    """
    ...
    theta_1, theta_2 = 0, 0
    if keypoint:
        key_point = keypoint * img_h
        if key_point > domain_min_y and key_point < domain_max_y:
            key_x = interp(key_point)
            theta_2 = math.atan((key_point - sample_ys_inside_domain[-1]) / (interp_xs[-1] - key_x + 1e-5)) / math.pi
            theta_1 = math.atan((-key_point + sample_ys_inside_domain[0]) / (-interp_xs[0] + key_x + 1e-5)) / math.pi
        elif key_point <= domain_min_y:
            theta_1 = math.atan((sample_ys_inside_domain[0] - sample_ys_inside_domain[-1]) / (
                    interp_xs[-1] - interp_xs[0] + 1e-5)) / math.pi
            theta_2 = theta_1
        else:
            theta_1 = math.atan((sample_ys_inside_domain[0] - sample_ys_inside_domain[-1]) / (
                    interp_xs[-1] - interp_xs[0] + 1e-5)) / math.pi
            theta_2 = theta_1
        theta_1 = theta_1 if theta_1 > 0 else 1 - abs(theta_1)
        theta_2 = theta_2 if theta_2 > 0 else 1 - abs(theta_2)
    ...
    return xs_outside_image, xs_inside_image, theta_1, theta_2
