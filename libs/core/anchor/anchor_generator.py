...
class CLRerNetAnchorGenerator(nn.Module):
    ...
    def __init__(
        ...
        if keypoint !=0 :
            self.prior_embeddings = nn.Embedding(self.num_priors, 4)
        else:
    ...
    def init_anchors(self):
        ...
        for i in range(left_priors_nums):
            ...
            if self.keypoint != 0:
                nn.init.constant_(self.prior_embeddings.weight[i, 3], 0.16 if i % 2 == 0 else 0.32)
        ...
        for i in range(left_priors_nums, left_priors_nums + bottom_priors_nums):
            ...
            if self.keypoint != 0:
                nn.init.constant_(self.prior_embeddings.weight[i, 3], 0.2 * (i % 4 + 1))

        for i in range(left_priors_nums + bottom_priors_nums, self.num_priors):
            nn.init.constant_(self.prior_embeddings.weight[i, 0], 1 - ((i - left_priors_nums - bottom_priors_nums) // 2) * strip_size)
            ...
            if self.keypoint != 0:
                nn.init.constant_(self.prior_embeddings.weight[i, 3], 0.68 if i % 2 == 0 else 0.84)
    ...
    def generate_anchors(self, anchor_params, prior_ys, sample_x_indices, img_w, img_h):
        ...
        if self.keypoint != 0:
            theta_low = anchor_params[..., 2].unsqueeze(-1).clone()
            theta_up = anchor_params[..., 3].unsqueeze(-1).clone()


            prior_ys_low = prior_ys[prior_ys > self.keypoint]
            prior_ys_up = prior_ys[prior_ys <= self.keypoint]
            len_low = len(prior_ys_low)
            len_up = len(prior_ys_up)
            theta_low = theta_low.repeat(1, len_low) * np.pi
            theta_up = theta_up.repeat(1, len_up) * np.pi

            x0 = anchor_params[..., 1].unsqueeze(-1).clone().repeat(1, len_low) * (img_w - 1)  # (192, 72)
            y0 = (anchor_params[..., 0].unsqueeze(-1).clone().repeat(1, len_low))  # (192, 72)

            tan_lane_low = torch.tan(theta_low + 1e-5)
            ys_low = prior_ys_low.repeat(num_anchors, 1)
            tan_lane_up = torch.tan(theta_up + 1e-5)
            ys_up = prior_ys_up.repeat(num_anchors, 1)

            anchor_xs_low = x0 - ((ys_low - y0) * img_h / tan_lane_low)
            mid_x = anchor_params[..., 1] * (img_w - 1) - ((self.keypoint - anchor_params[..., 0]) * img_h / tan_lane_low[..., 0])
            mid_x = mid_x.unsqueeze(-1).clone().repeat(1, len_up)
            anchor_xs_up = mid_x - ((ys_up - self.keypoint) * img_h / tan_lane_up)
            anchor_xs = torch.cat([anchor_xs_low, anchor_xs_up], -1)
        else:
            x0 = anchor_params[..., 1].unsqueeze(-1).clone().repeat(1, self.num_offsets) * (img_w - 1)  # (192, 72)
            y0 = (anchor_params[..., 0].unsqueeze(-1).clone().repeat(1, self.num_offsets))  # (192, 72)
