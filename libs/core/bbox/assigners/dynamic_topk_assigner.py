...
class DynamicTopkAssigner(BaseAssigner):
    ...
    def _clrernet_cost(self, predictions, targets, pred_xs, target_xs, eval_shape):
        """_summary_
    ...
    def assign(
        ...
        if predictions["anchor_params"].shape[-1] != 3:
            target_xs = targets[:, 7:] / (img_w - 1)  # abs -> relative
        else:
            target_xs = targets[:, 6:] / (img_w - 1)
        ...
        if self.cost_combination == 0:  # CLRNet
            ...
            cost = self._clrernet_cost(predictions, targets, pred_xs, target_xs, eval_shape)
        else:
